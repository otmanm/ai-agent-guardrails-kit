> **How to read this file.** This is a working comparison of the open and free dynamic-workflow tools and their built-in guardrails, **assessed from each tool's own documentation, source code, and public issues, not from a benchmark I ran against them.** The one thing in this repo that was actually run and proved is the hook test suite in [hooks/](hooks/) (18/18 passing) plus the incident itself. A reproducible cross-tool attack harness that records caught/missed per tool is the planned next step. See [VERIFICATION.md](VERIFICATION.md) for what is verified vs sourced vs open.
